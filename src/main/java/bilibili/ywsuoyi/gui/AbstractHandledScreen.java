...
public abstract class AbstractHandledScreen<T extends ScreenHandler> extends HandledScreen<T> {
    ...
    public AbstractHandledScreen(T handler, PlayerInventory inventory, Text title, Identifier texture) {
    ...
    public void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        if (this.hasIcon) {
            ...
            for (bilibili.ywsuoyi.gui.icon icon : this.icons) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (this.hasButton) {
            ...
            for (bilibili.ywsuoyi.gui.button b : this.buttons) {
                b.clicked = false;
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        ...
        scroll b2;
        for(var6 = this.scrolls.iterator(); var6.hasNext(); b2.clicked = false) {
            b2 = (scroll)var6.next();
        }
    ...
    public void drawMouseoverTooltip(MatrixStack matrices, int x, int y) {
        ...
        for (percentage p : this.percentages) {
            if (p.isclicked(x - this.x, y - this.y)) {
                String a = String.valueOf(this.getPvar(p.p, ((screenHandler) this.handler).e));
    ...
    public void renderPercentage(MatrixStack matrices, BlockEntity e) {
        int i = this.x;
        int j = this.y;
        Iterator<percentage> var5 = this.percentages.iterator();

        while(true) {
            label61:
            while(var5.hasNext()) {
                percentage p = var5.next();
                int a;
                switch(p.type) {
                    case 1:
